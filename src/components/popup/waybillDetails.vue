                        ...
                        <div class="news">
                            <div class="news-left" ref="copy">
                                <div class="CargoInformation">
                                    ...
                                    <div
                                        class="value"
                                    >{{news.wcd?news.wcd:""}}&ensp;{{news.ctm?news.ctm:""}}</div>
                                </div>
                                <div class="CargoInformation">
                                    <div class="key">货物信息：</div>
                                    <div
                                        class="value"
                                    >{{news.on?news.on+"，":""}}{{news.te==1?"整车，":"配货，"}}{{news.dw?news.dw+"吨":""}}{{news.dw?news.dw+"方":""}}</div>
                                </div>
                                <div class="VehicleDemand">
                                    <div class="key">车辆信息：</div>
                                    <div
                                        class="value"
                                    >{{news.ct?news.ct:""}}{{news.cl?news.cl+"，":""}}{{news.cn?news.cn+"，":""}}{{news.cne?news.cne:""}}{{news.cph?news.cph:""}}</div>
                                </div>
                                <div class="CargoInformation">
                                    <div class="key">运输时间：</div>
                                    <div
                                        class="value"
                                    >{{news.qzte?news.qzte:""}} &ensp;至 &ensp;{{news.qxte?news.qxte:""}}</div>
                                </div>
                                <div class="CargoInformation">
                                    <div class="key">运费信息：</div>
                                    <div
                                        class="value"
                                    >{{news.yf?"货主运费："+news.yf+"，":""}}{{news.tn?"税费："+news.tn+"，":""}}{{news.dyf?"司机运费："+news.dyf:""}}</div>
                                </div>
                                <div class="CargoInformation">
                                    <div class="key">已付运费：</div>
                                    <div
                                        class="value"
                                    >{{news.pyf?"已收货主运费："+news.pyf:""}}&ensp;&ensp;{{news.pdyf?"已付司机运费："+news.pdyf:""}}</div>
                                </div>
                            </div>
                        </div>
...
export default {
    ...
    methods: {
        ...
        //信息赋值
        assignment() {
            this.news.wcd = this.information.wcd;
            this.news.ctm = this.information.ctm;
            this.news.te = this.information.te;
            this.news.dw = this.information.dw;
            this.news.dv = this.information.dv;
            this.news.on = this.information.on;
            this.ctType.map((item, index) => {
                if (this.information.ct == index + 1) {
                    this.information.ctText = item;
                }
            });
            this.clType.map((item, index) => {
                if (this.information.cl == index + 1) {
                    this.information.clText = item;
                }
            });
            this.news.ct = this.information.ctText;
            this.news.cl = this.information.clText;
            this.news.cn = this.information.cn;
            this.news.cne = this.information.cne;
            this.news.cph = this.information.cph;

            this.news.qzte = this.information.qzte;
            this.news.qxte = this.information.qxte;
            this.news.yf = this.information.yf;
            this.news.tn = this.information.tn;
            this.news.dyf = this.information.dyf;
            this.news.pyf = this.information.pyf;
            this.news.pdyf = this.information.pdyf;
        },
        open1(text) {
            this.$message({
                message: text,
